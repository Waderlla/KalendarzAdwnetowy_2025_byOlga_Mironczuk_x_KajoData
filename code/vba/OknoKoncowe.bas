...
' z kolumny "Final" (nazwa zaczyna się na "f", np. f3) i pokazujemy tylko jego.
' Pozostałe f* ukrywamy. Innych kształtów nie dotykamy.
'--------------------------------------------------------------------

'=== KONFIGURACJA ===
Private Const TBL_SHEET As String = "tajne zapiski elfów"   ' Arkusz z tabelą sterującą
Private Const CAL_SHEET As String = "kalendarz"             ' Arkusz z kalendarzem

Private Const HDR_PERCENT As String = "ProcentDocelowy"     ' Kolumna z wartoącią procentową (0–1)
Private Const HDR_FINAL As String = "Final"                 ' Kolumna z nazwą obrazka f* (np. f5)

Private Const PERCENT_CELL As String = "D27"                ' Komórka z bieżącym procentem (0–1)
...
Public Sub UpdateFinalByPercent()
    ...
    ' 0) Bramka daty – pokaż f* dopiero OD dnia z D28.
    '    Jeśli D28 nie jest prawdziwą datą (tekst albo puste) – bramkę ignorujemy.
    ...
    ' 3) Szukamy wiersza, którego ProcentDocelowy jest najbliżej targetPct
    ...
    ' 5) Przełącz widoczność tylko dla grupy f*
...
'--------------------------------------------------------------------
' Funkcja pomocnicza: zwraca numer kolumny, której nagłówek (wiersz 1)
' jest równy podanemu tekstowi (bez rozróżniania wielkości liter).
' Gdy brak - zwraca 0.
'--------------------------------------------------------------------
