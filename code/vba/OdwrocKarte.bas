...
'=== KONFIG ===
Private Const TBL_SHEET As String = "tajne zapiski elfów"   ' tabela sterująca
Private Const CAL_SHEET As String = "kalendarz"             ' arkusz z kształtami

Private Const HDR_START As String = "PoczatkowaData"        ' data startu (dzień odkrycia)
Private Const HDR_BACK  As String = "TylKarty"              ' nazwa obrazka tk* (np. tk7)

' Ukryj tk*, dla których PoczatkowaData <= dzisiaj. Pozostałe pokaż.
    ...
    For r = 2 To lastRow
        ...
        If Len(nm) > 0 Then
            ...
            If IsDate(v) Then
                d = DateValue(CDate(v))
                If d <= today Then
                    toHide.Add nm     ' dzień nadszedł -> ukryj tył
                Else
                    toShow.Add nm     ' jeszcze przed startem -> pokaż tył
                End If
            End If
        End If
    Next r

    ' Przełącz widoczność tylko dla tk*
    Dim shp As Shape
